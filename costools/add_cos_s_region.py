...
def add_s_region(input_file, dry_run):
    ...
    with fits.open(input_file, mode=open_mode) as f1:
        ...
        for n_ext, extension in enumerate(f1[1:]):
            ...
            if exthdr['extname'] in ['SCI', 'EVENTS', 'ACQ']:
                try:
                    ...
                    cosutil.printContinuation('in extension {} header of file {}'.format(n_ext, input_file))
                    continue
                diameter = 2.5 / 3600.0
                ...
                if not dry_run:
                    try:
                        ...
                        cosutil.printMsg("Using existing S_REGION keyword in file {}".format(input_file))
                    except KeyError:
                        ext1hdr.insert('PA_APER', ('S_REGION', s_region, 'Spatial extent of the observation'), after=True)
                        cosutil.printMsg('Added S_REGION keyword to {}'.format(input_file))
                    cosutil.printMsg("Setting value to '{}'".format(s_region))
                else:
                    cosutil.printMsg("S_REGION for file {} = '{}'".format(input_file, s_region))
